...
function Large(props) {
	...
	const otherHeaders = headers.filter(
		header => header !== idHeader && header !== mainHeader && header !== imageHeader,
	);
				...
				{collection.map((item, index) => (
					<li key={index}>
						<div className="list-large-content">
							<h2>
								<Value header={idHeader} item={item} name={item[mainHeader.key]} />
								.
								<Value header={mainHeader} item={item} name={item[mainHeader.key]} />
							</h2>
							<dl>
								{otherHeaders.map(header => (
									<div key={header.key} className="list-large-keyvalue">
										<dt>{header.label}</dt>
										<dd>
											<Value header={header} item={item} name={item[mainHeader.key]} />
										</dd>
									</div>
								))}
							</dl>
						</div>
						<Value header={imageHeader} item={item} name={item[mainHeader.key]} />
					</li>
				))}
			</ul>
		</div>
